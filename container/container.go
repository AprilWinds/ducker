...
func (c *container) start() error {
	...
	// 1. 准备子进程
	cmd, syncRead, syncWrite, logFile, err := c.prepareChildProcess()
	if err != nil {
		return err
	}
	...
	if !c.Detach {
		...
		// detach 模式：后台 goroutine 等待进程退出并清理
		go func() {
			cmd.Wait()
			if logFile != nil {
				logFile.Close()
...
// prepareChildProcess 准备子进程命令，返回 cmd、同步管道读端和写端、日志文件
func (c *container) prepareChildProcess() (*exec.Cmd, *os.File, *os.File, *os.File, error) {
	syncRead, syncWrite, err := os.Pipe()
	if err != nil {
		return nil, nil, nil, nil, fmt.Errorf("create sync pipe: %w", err)
	}
	...
	return cmd, syncRead, syncWrite, logFile, nil
}

// setupIO 配置子进程的输入输出
func (c *container) setupIO(cmd *exec.Cmd) (*os.File, error) {
	if c.Detach {
		// 后台模式：输出到日志文件
...
func (c *container) logs(follow bool, tailLines int) error {
	...
	// Seek to end of file to avoid re-reading or skipping data due to Scanner's buffering
